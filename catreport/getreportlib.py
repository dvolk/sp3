...
def get_report(reports_db, fs, pipeline_run_uuid, sample_name):
    ...
    def get_report_file(report_uuid):
        ...
        if not r:
            logging.warning(
                f"gridfs file {report_uuid} for run {pipeline_run_uuid} sample {sample_name} not found or empty"
            )
            return None
        try:
            return r.decode()
    ...
    if r:
        ...
        try:
            ...
            report_data["resistance"]["finished_epochtime"] = int(
                r["finished_epochtime"]
            )
    ...
    if r:
        ...
        if f:
            ...
            report_data["mykrobe_speciation"]["finished_epochtime"] = int(
                r["finished_epochtime"]
            )
    """
    ...
    if r:
        ...
        if "samples" in j and sample_name in j["samples"]:
            report_data["run_distmatrix"]["data"]["samples"][sample_name] = j[
                "samples"
            ][sample_name]
        report_data["run_distmatrix"]["finished_epochtime"] = int(
            r["finished_epochtime"]
        )
    """
    ...
    if r:
        ...
        if f:
            ...
            report_data["kraken2_speciation"]["finished_epochtime"] = int(
                r["finished_epochtime"]
            )
    """
    end speciation report
    ...
    if r:
        ...
        if f:
            ...
            report_data["pick_reference"]["finished_epochtime"] = int(
                r["finished_epochtime"]
            )
    """
    end speciation report
    ...
    if r:
        ...
        if f:
            ...
            report_data["samtools_qc"]["finished_epochtime"] = int(
                r["finished_epochtime"]
            )
    """
    ...
    if r:
        ...
        report_data["nfnvm_nanostats_qc"]["finished_epochtime"] = int(
            r["finished_epochtime"]
        )
    """
    ...
    if r:
        ...
        report_data["nfnvm_kronareport"]["finished_epochtime"] = int(
            r["finished_epochtime"]
        )
    """
    ...
    """
    logging.warning(
        f"Calling get_samples_cov_names with report type: nfnvm_map2coverage_report"
    )
    samples_cov_names = list(
        get_samples_cov_names(
            reports_db, pipeline_run_uuid, sample_name, "nfnvm_map2coverage_report"
        )
    )
    ...
    if r:
        ...
        report_data["nfnvm_flureport"]["finished_epochtime"] = int(
            r["finished_epochtime"]
        )
    """
    ...
    if r:
        ...
        report_data["nfnvm_viralreport"]["finished_epochtime"] = int(
            r["finished_epochtime"]
        )
    """
    ...
    """
    samples_resistance_names = list(
        get_samples_cov_names(
            reports_db, pipeline_run_uuid, sample_name, "nfnvm_resistance"
        )
    )
    ...
    if samples_resistance_names:
        ...
        for sample_resistance_name_string in samples_resistance_names:
            ...
            r = get_report_for_type(
                pipeline_run_uuid, sample_resistance_name, "nfnvm_resistance"
            )
            ...
            if r:
                ...
                report_data["nfnvm_resistance_report"]["finished_epochtime"] = r[
                    "finished_epochtime"
                ]
                resistance = dict()
    ...
    logging.warning(
        f"get_report({pipeline_run_uuid}, {sample_name}) finished in {(b-a).total_seconds()} s"
    )
