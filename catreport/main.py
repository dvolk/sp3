...
def db_get_report_for_type(pipeline_run_uuid, sample_name, report_type):
    return reports_db.find_one(
        ...
        sort=[("added_epochtime", pymongo.DESCENDING)],
    )


def db_get_queue(report_type):
    return reports_db.find(
        {"status": "queued", "type": report_type},
        sort=[("added_epochtime", pymongo.DESCENDING)],
    )


def db_update_report_result(
    report_started_epochtime, report_finished_epochtime, report_status, report_uuid, gid
):
    reports_db.update(
        {"uuid": report_uuid},
        {
...
def save_report_file(report_uuid, report_content):
    ...
    prefix = pathlib.Path(
        f"/work/reports/catreport/reports/{report_uuid[0:2]}/{report_uuid[2:4]}/"
    )
...
def report_thread_factory(report_type, make_report_function):
    ...
    while True:
        ...
        for row in rows:
            ...
            logging.warning(
                f"thread {report_type}: working on run {row['pipeline_run_uuid']} sample ({row['sample_name']})"
            )
            report_content = make_report_function(
                row["uuid"],
                row["sample_filepath"],
                row["sample_name"],
            ...
            logging.warning(
                f"thread {report_type}: finished run {row['pipeline_run_uuid']} sample ({row['sample_name']}) in {(b-a).total_seconds()} s"
            )
        time.sleep(15)


def make_trivial_copy_report(
    report_uuid, sample_filepath, sample_name, pipeline_run_uuid
):
...
def make_within_run_distreport(
    report_uuid, sample_filepath, sample_name, pipeline_run_uuid
):
    os.system(
        f"python3 distmatrixsp3input.py /work/output/{pipeline_run_uuid} | python3 distmatrix.py > /tmp/distreport_{report_uuid}.txt"
    )
    with open(f"/tmp/distreport_{report_uuid}.txt") as f:
        return f.read()


def make_resistance_report(
    report_uuid, sample_filepath, sample_name, pipeline_run_uuid
):
    """
    ...
    """
    os.system(
        f"cd /work/reports/resistanceapi/vcfs; ln -s {sample_filepath} {report_uuid}.vcf"
    )
...
def get_report(pipeline_run_uuid, sample_name):
    return json.dumps(
        getreportlib.get_report(reports_db, fs, pipeline_run_uuid, sample_name)
    )


def main():
    threading.Thread(
        target=report_thread_factory, args=("resistance", make_resistance_report)
    ).start()
    threading.Thread(
        target=report_thread_factory,
        args=("mykrobe_speciation", make_trivial_copy_report),
    ).start()
    threading.Thread(
        target=report_thread_factory,
        args=("kraken2_speciation", make_trivial_copy_report),
    ).start()
    threading.Thread(
        target=report_thread_factory, args=("pick_reference", make_trivial_copy_report)
    ).start()
    threading.Thread(
        target=report_thread_factory, args=("samtools_qc", make_trivial_copy_report)
    ).start()

    threading.Thread(
        target=report_thread_factory,
        args=("nfnvm_nanostats_qc", make_trivial_copy_report),
    ).start()
    threading.Thread(
        target=report_thread_factory,
        args=("nfnvm_kronareport", make_trivial_copy_report),
    ).start()
    threading.Thread(
        target=report_thread_factory, args=("nfnvm_flureport", make_trivial_copy_report)
    ).start()
    threading.Thread(
        target=report_thread_factory,
        args=("nfnvm_viralreport", make_trivial_copy_report),
    ).start()
    threading.Thread(
        target=report_thread_factory,
        args=("nfnvm_map2coverage_report", make_trivial_copy_report),
    ).start()

    threading.Thread(
        target=report_thread_factory,
        args=("nfnvm_resistance", make_trivial_copy_report),
    ).start()

    threading.Thread(
        target=report_thread_factory,
        args=("run_distmatrix", make_within_run_distreport),
    ).start()
