...
con.execute(
    "create table if not exists q (uuid primary key, type, status, added_epochtime, started_epochtime, finished_epochtime, pipeline_run_uuid, sample_name, sample_filepath, report_filename, software_versions)"
)
...
def db_get_report_for_type(
    cluster_instance_uuid, pipeline_run_uuid, sample_name, report_type
):
    with sql_lock, con:
        r = con.execute(
...
def db_get_queue(con, report_type):
    with sql_lock, con:
        rows = con.execute(
            "select * from q where status = 'queued' and type = ? order by added_epochtime desc limit 1",
            (report_type,),
        ).fetchall()
    return rows
...
def db_get_queued_report_count(con, report_type):
    with sql_lock, con:
        rows = con.execute(
            "select count(*) from q where type = ? and status = 'queued'",
            (report_type,),
        ).fetchone()
    return rows
...
def list_report():
    ...
    try:
        ...
        status = "failed"
    return json.dumps({"status": status, "details": detail, "data": data})
...
def get_report(pipeline_run_uuid, sample_name):
    return json.dumps(
        getreportlib.get_report(None, con, sql_lock, pipeline_run_uuid, sample_name)
    )
