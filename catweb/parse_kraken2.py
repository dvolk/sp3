...
def read_kraken2(kraken2_data, pct_threshold, num_threshold):
    ...
    for line in kraken2_data.splitlines():
        ...
        if (
            pc_frags_num >= pct_threshold and frags_rooted_num >= num_threshold
        ) or name == "Homo sapiens":
            if rank_code == "S":
                result["Species"].append(
                    {
                        "reads": frags_rooted_num,
                        "percentage": pc_frags_num,
                        "name": name,
                        "taxon": ncbi_taxon_id,
                    }
                )
...
def sort_result(result, pct_threshold, num_threshold):
    if len(result["Family"]) == 0:
        ...
        result["Family"] = sorted(
            result["Family"], key=lambda k: k["reads"], reverse=True
        )
        if (result["Family"][0]["name"] == "Mycobacteriaceae") and result["Family"][0][
            "reads"
        ] >= 100000:
    ...
    if len(result["Species"]) == 0:
        ...
        result["Species"] = sorted(
            result["Species"], key=lambda k: k["reads"], reverse=True
        )
        species = result["Species"]
        non_human_species = list(
            filter(lambda item: item["name"] != "Homo sapiens", species)
        )
        if len(non_human_species) > 1:
            result["Warnings"][
                "species notes"
            ] = f"Sample is mixed or contaminated (contains reads from multiple non-human species)."
    ...
    if len(result["Genus"]) == 0:
        ...
        result["Genus"] = sorted(
            result["Genus"], key=lambda k: k["reads"], reverse=True
        )
    ...
    if len(result["Species complex"]) == 0:
        ...
        result["Species complex"] = sorted(
            result["Species complex"], key=lambda k: k["reads"], reverse=True
        )
        if len(result["Species complex"]) > 1:
            result["Warnings"][
                "Species complex notes"
            ] = f"Sample contains multiple mycobacterial species complexes."
        if (
            result["Species complex"][0]["name"] == "Mycobacterium tuberculosis complex"
        ) and result["Species complex"][0]["reads"] >= 100000:
            result["Warnings"]["mykrobe"] = True

    if (
        isinstance(result["Family"], list)
        and len(result["Family"]) > 0
        and isinstance(result["Genus"], list)
        and len(result["Genus"]) > 0
        and isinstance(result["Species"], list)
        and len(result["Species"]) > 0
    ):
        if "Mycobact" in result["Family"][0]["name"] and (
            "Mycobact" not in result["Genus"][0]["name"]
            or "Mycobact" not in result["Species"][0]["name"]
        ):
            result["Warnings"][
                "mycobact notes"
            ] = f"Top family classification is mycobacterial, but this is not consistent with top genus or species classifications."

    if ("Mycobact notes" in result["Warnings"].keys()) or (
        "species notes" in result["Warnings"].keys()
    ):
        result["Warnings"]["mykrobe"] = False
        result["Warnings"][
            "notes"
        ] = f"Mykrobe report might be produced, please be cautious with the result."

    if result["Warnings"]["mykrobe"] == True:
        result["Warnings"][
            "notes"
        ] = f"For higher-resolution classification, see Mykrobe report."
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        "--pct_reads",
        default=1,
        help="threshold of percentage of reads, default 1%",
    )
    parser.add_argument(
        "-n",
        "--number_reads",
        default=10000,
        help="threshold of number of reads, default 10k",
    )
    parser.add_argument(
        "-o",
        "--output_file",
    ...
    print(
        f"File:{args.kraken_file}, pct_reads: {args.pct_reads}, number_reads: {args.number_reads}"
    )
