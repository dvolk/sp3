...
SP3_VERSION=$(git describe --tags)
popd

# Get catsgo version
pushd /home/ubuntu/catsgo
CATSGO_VERSION=$(git describe --tags)
popd
...
COVID_PIPE_VERSION=$(git describe --tags)
popd
...
Viridian version - ${VIRIDIAN_VERSION}" >> /home/ubuntu/release_notes/$(date +%F_%T)_versions.txt