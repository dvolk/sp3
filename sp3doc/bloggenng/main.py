...
for post_path in post_paths:
    ...
    post_out.html.head.insert(
        0, BS(f"<title>{ post_out.h2.text }</title>", features="lxml").html.head.title
    )
    post_out.h2.replaceWith(
        BS(
            f'<h2><a href="./{ post_path.stem }.html">{ soup.h2.text }</a></h2>',
            features="lxml",
        ).body.next
    )
    ...
    posts.append(str(soup))

soup = BS(header + "".join(posts) + footer, features="lxml")
soup.html.head.insert(
    0, BS(f"<title>{ soup.h1.text }</title>", features="lxml").html.head.title
)
print(str(soup))
