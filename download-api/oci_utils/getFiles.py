...
def getFiles(bucket, user, keyfile):
    ...
    object_list = object_storage_client.list_objects(namespace, bucket, prefix="meta")

    # to try and get more than 1000 files, doesn't work yet
    while object_list.has_next_page:
        object_list.extend(
            object_storage_client.list_objects(
                namespace, bucket, prefix="meta", start=object_list.nextStartWith
            )
        )
    ...
    for o in object_list.data.objects:
        head_object_response = object_storage_client.head_object(
            namespace_name=namespace, bucket_name=bucket, object_name=o.name
        )
        run = head_object_response.headers["opc-meta-run"]
        sampleID = head_object_response.headers["opc-meta-sampleID"]
        d[o.name] = {"name": o.name, "run": run, "sampleID": sampleID}
    return d
...
def groupFiles(d):
    ...
    for f in d:
        runs.setdefault(d[f]["run"], {}).setdefault(d[f]["sampleID"], []).append(
            d[f]["name"]
        )
...
if __name__ == "__main__":
    ...
    parser = ArgumentParser(
        description="Create CSV files from OCI object store fastq files"
    )
    ...
    parser.add_argument(
        ...
        "--user",
        required=False,
        default="ocid1.user.oc1..aaaaaaaaavnaohgvyjwnvmkiigjnpiolola2zhamt7npvurn3vlxb55rfjqa",
        help="user",
    )
    parser.add_argument(
        "-k",
        "--keyfile",
        required=False,
        default="/home/ubuntu/.oci/oci_api_key.pem",
