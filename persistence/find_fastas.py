...
def copy_fasta_files(site_uuid, run_uuid, sample_ref, area, existing_fasta_files):
    ...
    for f in fasta_files:
        ...
        if is_sample_correct_ref(site_uuid, run_uuid, sample_name, sample_ref):
            dest = Path(
                f"/work/persistence-catwalk/fasta/{area}/{site_uuid}/{sample_ref}/{run_sample_fasta_name}"
            )
            if not dest.exists():
                count += 1
...
def insert_fasta_files_to_catwalk(area, site_uuid, sample_ref):
    samples_already_there = set(
        [x.name for x in Path("/home/ubuntu/catwalk/test").glob("*")]
    )
    all_samples = list(
        Path("/work/persistence-catwalk/fasta").glob(f"{area}/*/{sample_ref}/*.fasta")
    )
...
def find_all_fasta_files(area, site_uuid, sample_ref):
    return {
        x.name
        for x in Path(
            f"/work/persistence-catwalk/fasta/{area}/{site_uuid}/{sample_ref}"
        ).glob("*.fasta")
    }
