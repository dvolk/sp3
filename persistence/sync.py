...
def main():
    ...
    runs = con.execute(
        "select run_uuid from nfruns where status = 'OK' and cast(start_epochtime as integer) > ?",
        (last_synced,),
    ).fetchall()
    ...
    cmd(
        f"rsync            -a {' '.join(local_files_abs)} {store_host}:/work/persistence/{instance_id}/"
    )
    # TODO chunk local_files_rel in case it's too big
    cmd(
        f"rsync --relative -a {' '.join(local_files_rel)} {store_host}:/work/persistence/{instance_id}/"
    )

    # selectively sync output files (fasta and vcf files etc)
    if local_outputs and output_patterns:
        includes_files = " ".join(
            ["--include '" + include + "'" for include in output_patterns]
        )
        ...
        cmd(
            f"rsync --relative -a  {includes_str} {local_outputs_str} {store_host}:/work/persistence/{instance_id}/"
        )
