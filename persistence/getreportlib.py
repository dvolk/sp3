...
def get_report(cluster_instance_uuid, con, pipeline_run_uuid, sample_name):
    ...
    def get_report_for_type(con, pipeline_run_uuid, sample_name, report_type):
        with con:
            r = con.execute(
                'select * from q where status = "done" and pipeline_run_uuid = ? and sample_name = ? and type = ? order by added_epochtime desc',
                (pipeline_run_uuid, sample_name, report_type),
            ).fetchall()
            logging.warning(r)
        if r:
            return r[0]
    ...
    if cluster_instance_uuid:
        ...
        if cluster_instance_uuid in cons:
            ...
            con = sqlite3.connect(
                f"/work/persistence/{ cluster_instance_uuid }/db/catreport.sqlite",
    ...
    if r:
        ...
        report_resistance_filepath = (
            f"{ reports_directory }/{report_resistance_guid}.json"
        )
        ...
        try:
            ...
            report_data["resistance"]["data"] = resistance_help.resistance_postprocess(
                report_data
            )
    ...
    if r:
        ...
        report_mykrobe_speciation_filepath = (
            f"{ reports_directory }/{report_mykrobe_speciation_guid}.json"
        )
    ...
    if r:
        ...
        report_kraken2_speciation_filepath = (
            f"{ reports_directory }/{report_kraken2_speciation_guid}.json"
        )
    ...
    if r:
        ...
        report_pick_reference_filepath = (
            f"{ reports_directory }/{report_pick_reference_guid}.json"
        )
    ...
    if r:
        ...
        report_samtools_qc_filepath = (
            f"{ reports_directory }/{report_samtools_qc_guid}.json"
        )
    ...
    if r:
        ...
        report_nfnvm_nanostats_qc_filepath = (
            f"{ reports_directory }/{report_nfnvm_nanostats_qc_guid}.json"
        )
    ...
    if r:
        ...
        report_nfnvm_krona_downloadpath = (
            f"{pipeline_run_uuid}/selReference_Out/{sample_name}_classkrona.html"
        )
    ...
    """
    samples_cov_names = get_samples_cov_names(
        con, pipeline_run_uuid, sample_name, "nfnvm_map2coverage_report"
    )
    ...
    if samples_cov_names != None:
        ...
        for sample_cov_name_string in samples_cov_names:
            ...
            r = get_report_for_type(
                con, pipeline_run_uuid, sample_cov_name, "nfnvm_map2coverage_report"
            )
            ...
            if r:
                ...
                report_nfnvm_cov_downloadpath = (
                    f"{pipeline_run_uuid}/mapping2_Out/{sample_cov_name}_cov.png"
                )
                ...
                report_data["nfnvm_map2coverage_report"]["finished_epochtime"] = int(
                    r[5]
                )
                report_data["nfnvm_map2coverage_report"]["data"].append(
                    sample_ref_files
                )
                logging.warning(report_nfnvm_cov_downloadpath)
    ...
    if r:
        ...
        report_nfnvm_flureport_filepath = (
            f"{ reports_directory }/{report_nfnvm_flureport_guid}.json"
        )
    ...
    if r:
        ...
        report_nfnvm_viralreport_filepath = (
            f"{ reports_directory }/{report_nfnvm_viralreport_guid}.json"
        )
    ...
    """
    samples_resistance_names = get_samples_cov_names(
        con, pipeline_run_uuid, sample_name, "nfnvm_resistance"
    )
    ...
    if samples_resistance_names != None:
        ...
        for sample_resistance_name_string in samples_resistance_names:
            ...
            r = get_report_for_type(
                con, pipeline_run_uuid, sample_resistance_name, "nfnvm_resistance"
            )
            ...
            if r:
                ...
                report_nfnvm_resistance_downloadpath = (
                    f"{ reports_directory }/{ report_nfnvm_resistance_guid }"
                )
