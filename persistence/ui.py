    ...
    if request.method == "POST":
        ...
        for ldap_domain, ldap_dict in auth_ldap.items():
            if form_username in ldap_dict["authorized_users"]:
                ...
                is_ok = authenticate.check_ldap_authentication(
                    form_username, form_password, ldap_host
                )
                if is_ok:
                    ...
                    print(
                        f"invalid credentials for ldap user {form_username} host {ldap_host}"
                    )
                    return redirect("/")
        else:
...
def get_instances(persistence_dir):
    ...
    for d in ds:
        ...
        ret.append({"dir": d, **instance_conf, "updated": updated})
    return ret
...
def make_runs_name_map():
    ...
    for instance in get_instances("/work/persistence"):
        ...
        with sqlite3.connect(
            f"/work/persistence/{instance_id}/db/catweb.sqlite"
        ) as con:
...
def api_cw_get_neighbours(run_uuid_and_sample_name, snp_distance):
    return requests.get(
        f"http://localhost:5000/neighbours/{ run_uuid_and_sample_name }/{ snp_distance }"
    ).text
...
def list_instances():
    ...
    return render_template("main.template", instances=instances, labkey_url=labkey_url)
...
def browse(cluster_id):
    ...
    runs = con.execute(
        'select * from nfruns  where status="OK" order by start_epochtime desc'
    ).fetchall()
    runs = org_filter_runs(get_user_organisation(), runs)

    return render_template(
        "cluster_browse.template",
        runs=runs,
...
def get_report(cluster_uuid, run_uuid, dataset_id):
    catpile_resp = None
    report_data = getreportlib.get_report(cluster_uuid, None, run_uuid, dataset_id)[
        "data"
    ]["report_data"]
...
def cluster_run_details(cluster_id, run_uuid):
    ...
    reports = con2.execute(
        "select distinct sample_name from q where pipeline_run_uuid = ?", (run_uuid,)
    ).fetchall()

    for report in reports:
        try:
            report["finished_time_str"] = time.strftime(
                "%Y/%m/%d %H:%M", time.localtime(int(report["finished_epochtime"]))
            )
        except:
