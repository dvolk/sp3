...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="whether to ignore the STATUS field in the vcf (e.g. necessary for some versions of Clockwork VCFs)",
    )
    parser.add_argument(
        "--ignore_vcf_filter",
        action="store_true",
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="print progress statements to STDOUT to help debugging",
    )
    ...
    if options.catalogue_file is not None:
        ...
        resistance_catalogue = piezo.ResistanceCatalogue(
            options.catalogue_file, prediction_subset_only=True
        )

    if options.debug:
        print(
            "Creating a sample Genome object by copying the reference Genome object..."
        )
    ...
    sample_genome.apply_vcf_file(
        show_progress_bar=options.progress,
    ...
    if VARIANT is not None:
        ...
        # set the index
        VARIANT.set_index(
            ["UNIQUEID", "VARIANT", "IS_SNP"], inplace=True, verify_integrity=True
        )
    ...
    for gene in reference_genome.gene_names:
        ...
        mutations = sample_genome.genes[gene].table_mutations_wrt(
            reference_genome.genes[gene]
        )
    ...
    if MUTATIONS is not None:
        ...
        MUTATIONS[
            ["IS_SYNONYMOUS", "IS_NONSYNONYMOUS", "IS_HET", "IS_NULL"]
        ] = MUTATIONS.apply(mutations_assign_booleans, axis=1)

        # calculate the number of nucleotide changes required for this mutation
        MUTATIONS["NUMBER_NUCLEOTIDE_CHANGES"] = MUTATIONS.apply(
            mutations_count_number_nucleotide_changes, axis=1
        )
        ...
        # set the index
        MUTATIONS.set_index(
            ["UNIQUEID", "GENE", "MUTATION"], inplace=True, verify_integrity=True
        )
    ...
    if options.catalogue_file is not None and MUTATIONS is not None:
        ...
        MUTATIONS_IN_CATALOGUE = MUTATIONS.loc[
            MUTATIONS.GENE.isin(resistance_catalogue.catalogue.genes)
        ]

        EFFECTS_dict = {}
        EFFECTS_counter = 0

        for gene_name, mutation_name in zip(
            MUTATIONS_IN_CATALOGUE.GENE, MUTATIONS_IN_CATALOGUE.MUTATION
        ):

            try:
                valid = reference_genome.valid_gene_mutation(
                    gene_name + "@" + mutation_name
                )
            except:
                valid = False
            ...
            if prediction != "S":
                ...
                for drug_name in prediction:
                    ...
                    if (
                        drug_name
                        and prediction[drug_name] == "S"
                        and phenotype[drug_name] == "S"
                    ):
                    ...
                    # (again for completeness including the superfluous state)
                        ...
                        drug_name
                        and prediction[drug_name] == "U"
                        and phenotype[drug_name] in ["S", "U"]
                    ):
                    ...
                    EFFECTS_dict[EFFECTS_counter] = [
                        sample_genome.name,
                        gene_name,
                        mutation_name,
                        resistance_catalogue.catalogue.name,
                        drug_name,
        ...
        EFFECTS = pandas.DataFrame.from_dict(
            EFFECTS_dict,
            orient="index",
            columns=[
                "UNIQUEID",
        ...
        EFFECTS.set_index(
            ["UNIQUEID", "DRUG", "GENE", "MUTATION", "CATALOGUE_NAME"], inplace=True
        )
    ...
    if options.catalogue_file is not None:
        print(
            "%40s %s"
            % ("Reference genome", resistance_catalogue.catalogue.genbank_reference)
        )
