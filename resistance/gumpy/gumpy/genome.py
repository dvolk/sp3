...
from gumpy import Gene, Genotype
...
class Genome(object):
    def __init__(
        ...
        assert (genbank_file is not None) or (
            fasta_file is not None
        ), "one of a GenBank file or a FASTA file  must be specified!"
        assert (
            default_promoter_length >= 0
        ), "the promoter length must be a positive integer!"
        assert isinstance(
            default_promoter_length, int
        ), "the promoter length must be a positive integer!"
        ...
        self.bases_to_integer, self.genome_coding_integers = numpy.unique(
            self.genome_coding_strand, return_inverse=True
        )
    ...
    def set_member_variables_from(self, genbank_file=None):
        ...
        self.genome_coding_strand = numpy.array(
        ...
        self.version = (
            reference_genome.annotations["accessions"][0]
            + "."
            + str(reference_genome.annotations["sequence_version"])
        )
        ...
        if "sequence_version" in reference_genome.annotations.keys():
            self.sample_metadata["SEQUENCE_VERSION"] = reference_genome.annotations[
                "sequence_version"
            ]
    ...
    def load_genbank_file(
        ...
        for record in tqdm(reference_genome.features, disable=not (show_progress_bar)):
            ...
            if record.type in ["CDS", "rRNA"]:
                ...
                if gene_name is not None and (
                    gene_subset is None or gene_name in gene_subset
                ):

                    assert len(gene_name) <= 20, (
                        "Gene "
                        + gene_name
                        + " is too long at "
                        + str(len(gene_name))
                        + " chars; need to change numpy.zeros definiton U20 to match"
                    )
                    ...
                    if record.strand == 1:
                        ...
                        if previous_gene_reversed:
                            gene_mask = (
                                (self.genome_index > gene_start)
                                & (self.genome_index <= gene_end)
                                & (~self.genome_is_cds)
                            )
                        else:
                            gene_mask = (self.genome_index > gene_start) & (
                                self.genome_index <= gene_end
                            )
                        ...
                        promoter_mask = (
                            (self.genome_index > gene_start - default_promoter_length)
                            & (self.genome_index <= gene_start)
                            & (~self.genome_is_cds)
                            & (~self.genome_is_promoter)
                        )
                        ...
                        promoter_coding_numbering = (
                            self.genome_index[promoter_mask] - gene_start - 1
                        )

                        if codes_protein:
                            gene_coding_numbering = numpy.floor_divide(
                                self.genome_index[gene_mask] - gene_start + 2, 3
                            )
                        else:
                            gene_coding_numbering = (
                                self.genome_index[gene_mask] - gene_start
                            )
                        ...
                        promoter_mask = (
                            (self.genome_index > gene_end)
                            & (self.genome_index <= gene_end + default_promoter_length)
                            & (~self.genome_is_cds)
                            & (~self.genome_is_promoter)
                        )
                        ...
                        promoter_coding_numbering = -1 * (
                            self.genome_index[promoter_mask] - gene_end
                        )

                        if codes_protein:
                            gene_coding_numbering = -1 * (
                                numpy.floor_divide(
                                    self.genome_index[gene_mask] - gene_end - 1, 3
                                )
                            )
                        else:
                            gene_coding_numbering = -1 * (
                                self.genome_index[gene_mask] - gene_end
                            )

                        gene_coding_position = (
                            -1 * (self.genome_index[gene_mask] - gene_end) + 1
                        )
                    else:
                        raise TypeError(
                            "gene in GenBank file has strand that is not 1 or -1"
                        )
                    ...
                    self.genome_nucleotide_number[
                        promoter_mask
                    ] = promoter_coding_position
        ...
        self.gene_names = numpy.unique(
            self.genome_feature_name[self.genome_feature_name != ""]
        )
    ...
    def _infer_variant_table_booleans(self, row):
        ...
        if "indel" in row["VARIANT"]:
            ...
            if row["INDEL_LENGTH"] > 0:
                ...
                INDEL_2 = (
                    str(row["GENOME_INDEX"]) + "_del_" + str(-1 * row["INDEL_LENGTH"])
                )
        else:
            MUTATION_TYPE = "SNP"
            alt = row["VARIANT"][-1]
    ...
    def table_variants_wrt(self, other):
        ...
        for (r, i, a) in zip(
            VARIANTS_dict["REF"], VARIANTS_dict["GENOME_INDEX"], VARIANTS_dict["ALT"]
        ):
        ...
        VARIANTS_dict["GENOME_INDEX"] = numpy.append(
            VARIANTS_dict["GENOME_INDEX"], self.genome_index[mask]
        )
        VARIANTS_dict["ALT"] = numpy.append(VARIANTS_dict["ALT"], self.indel_alt[mask])
        for i in self.genome_index[mask]:
            VARIANTS_dict["VARIANT"] = numpy.append(
                VARIANTS_dict["VARIANT"], str(i) + "_indel"
            )
        VARIANTS_dict["GENE"] = numpy.append(
            VARIANTS_dict["GENE"], self.genome_feature_name[mask]
        )
        VARIANTS_dict["POSITION"] = numpy.append(
            VARIANTS_dict["POSITION"], self.genome_nucleotide_number[mask]
        )
        VARIANTS_dict["NUCLEOTIDE_NUMBER"] = numpy.append(
            VARIANTS_dict["NUCLEOTIDE_NUMBER"], self.genome_nucleotide_number[mask]
        )
        VARIANTS_dict["ELEMENT_TYPE"] = numpy.append(
            VARIANTS_dict["ELEMENT_TYPE"], self.genome_feature_type[mask]
        )
        VARIANTS_dict["AMINO_ACID_NUMBER"] = numpy.append(
            VARIANTS_dict["AMINO_ACID_NUMBER"], self.genome_amino_acid_number[mask]
        )
        VARIANTS_dict["INDEL_LENGTH"] = numpy.append(
            VARIANTS_dict["INDEL_LENGTH"], self.indel_length[mask]
        )
        VARIANTS_dict["COVERAGE"] = numpy.append(
            VARIANTS_dict["COVERAGE"], self.coverage[mask]
        )
        ...
        for field in self.genome_sequence_metadata:
            VARIANTS_dict[field] = numpy.append(
                VARIANTS_dict[field], self.genome_sequence_metadata[field][mask]
            )
        ...
        if len(VARIANTS_dict["POSITION"]) > 0:
            ...
            ] = VARIANTS_table.apply(self._infer_variant_table_booleans, axis=1)
            #
            ...
            VARIANTS_table["IS_FILTER_PASS"] = VARIANTS_table.apply(
                infer_filter_fail, axis=1
            )
            ...
            VARIANTS_table = VARIANTS_table.astype(
                {
                    "VARIANT": "str",
                    "REF": "str",
                    "ALT": "str",
            ...
            VARIANTS_table = VARIANTS_table.replace(
                {
                    "POSITION": 0,
                    "NUCLEOTIDE_NUMBER": 0,
                    "AMINO_ACID_NUMBER": 0,
    ...
    def at_index(self, index):
        ...
        assert (
            index <= self.genome_length
        ), "index must be less than the length of the genome!"
    ...
    def distance(self, other):
        ...
        return numpy.count_nonzero(
            self.genome_coding_strand != other.genome_coding_strand
        )
    ...
    def snp_distance(self, other):
        ...
        snps = 0
        for i in self.genome_coding_strand[
            self.genome_coding_strand != other.genome_coding_strand
        ]:
            if i in ["c", "t", "a", "g"]:
                snps += 1
    ...
    def iterate_records_in_vcf(
        ...
        for record in tqdm(
            vcf_reader, disable=not (show_progress_bar), total=lines_in_vcf
        ):
            ...
            for sample_idx, (sample_name, sample_info) in enumerate(
                ...
                (
                    ...
                    index,
                    alt_bases,
                ) = self._get_variant_for_genotype_in_vcf_record(genotype, record)
                        ...
                        for before, after in zip(ref_bases, alt_bases):
                            ...
                            if before != after:
                                ...
                                if filter_fail:
                                    ...
                                    if (
                                        self.focussed_indices is not None
                                        and index in self.focussed_indices
                                    ):
                                        if after != "x":
                                            after = "o"
                                    else:
                ...
                # HET calls
                    ...
                    self.cope_with_HET_calls(
                        alt_bases, ref_bases, index, sample_info, genotype, filter_fail
                    )

        return

    def deal_with_an_INDEL(
        self, alt_bases, ref_bases, sample_info, genotype, index, filter_fail
    ):
        ...
        assert indel_length != 0, (
            "REF: " + ref_bases + " and ALT: " + alt_bases + " same length?"
        )
        ...
        if filter_fail:
            ...
            # make the mutation
            self._permute_sequence(
                index,
                coverage,
                indel_length=indel_length,
                indel_bases=(ref_bases, alt_bases),
            )

        return

    def cope_with_HET_calls(
        self, alt_bases, ref_bases, index, sample_info, genotype, filter_fail
    ):
        """
        HET calls
        """
            ...
            if len(ref_bases) == len(alt):
                ...
                for before, after in zip(ref_bases, alt):
                    ...
                    if filter_fail:
                        ...
                        if (
                            self.focussed_indices is not None
                            and idx not in self.focussed_indices
                        ):
                            continue
                    ...
                    self.het_coverage[(mask, strand)] = sample_info["COV"][
                        genotype.call()[strand]
                    ]
                ...
                if filter_fail:
                    ...
                    if (
                        self.focussed_indices is not None
                        and index not in self.focussed_indices
                    ):
                        continue
    ...
    def update_main_sequence_to_show_HETs(self, show_progress_bar, ignore_filter):
        ...
        all_hets_mask = (self.het_variations[:, 0] != "") | (
            self.het_variations[:, 1] != ""
        )
        ...
        self.genome_sequence[
            self.genome_on_noncoding_strand
        ] = self.genome_noncoding_strand[self.genome_on_noncoding_strand]
    ...
    def prepare_for_vcf_read(self, metadata_fields=None):
        ...
        if self.metadata_fields is not None:
            ...
            for field in self.metadata_fields:
                self.genome_sequence_metadata[field] = numpy.zeros(
                    self.genome_length, float
                )
    ...
    def _permute_sequence(
        self, idx, coverage, after=None, indel_length=0, indel_bases=(None, None)
    ):
        ...
        if after is not None:
            ...
            assert after in ["a", "t", "c", "g", "x", "z", "o"], (
                "passed base " + after + " not one of a,t,c,g,z,x,o"
            )
    ...
    def save_fasta(
        ...
        if chars_per_line is not None:
            assert (
                chars_per_line > 0
            ), "number of characters per line in the FASTA file must be an integer!"
    ...
    def _get_variant_for_genotype_in_vcf_record(
        ...
        if genotype.is_reference():
            ...
            variant = (
                record.alleles[genotype.call1].lower(),
                record.alleles[genotype.call2].lower(),
            )
    ...
    def _is_record_invalid(
        self, ignore_filter: bool, record: pysam.VariantRecord
    ) -> bool:
        """
    ...
    def valid_genome_variant(self, genome_variant: str):
        ...
        assert (
            genome_variant is not None
        ), "a genome_variant must be specified! e.g. FIXME"

        # since the smallest variant is '1a>c'
        assert (
            len(genome_variant) >= 4
        ), "a genome variant must have at least 4 characters e.g. 3a>c"
        assert (
            genome_variant[-2] == ">"
        ), "a genome variant must be of the form 3a>c where 3 is the 1-based position in the genome"
        ...
        assert (
            before != after
        ), "before and after are identical hence this is not a variant!"
        ...
        assert before == self.genome_sequence[mask][0], (
            "base in genome is "
            + self.genome_sequence[mask][0]
            + " but specified base is "
            + before
        )

        # check that the base to be mutated to is valid (z=het, ?=any other base according to the grammar)
        assert after in ["c", "t", "g", "a", "?", "z", "x", "o"], (
            after + " is not a valid nucleotide!"
        )
    ...
    def convert_variant_to_mutation(self, gene_variant: str, validate=True):
        ...
        assert alt_base in [
            ...
            "z",
            "o",
        ], "alt base is not a, t, c, g, z, x or o!"
        ...
        assert tmp_gene.sequence[mask] == ref_base, (
            "provided reference base "
            + ref_base
            + " does not match "
            + tmp_gene.sequence[mask][0]
            + " which is what is in the genome"
        )
        ...
        ref = tmp_gene.amino_acid_sequence[
            tmp_gene.amino_acid_numbering == amino_acid_position
        ][0]
        ...
        alt = tmp_gene.amino_acid_sequence[
            tmp_gene.amino_acid_numbering == amino_acid_position
        ][0]
    ...
    def valid_gene_mutation(self, mutation):
        ...
        assert len(cols) in [1, 2, 3], (
            "mutation " + mutation + " not in correct format!"
        )
        ...
        if len(cols) == 1:
            ...
            if "*" in cols[0]:
                ...
                assert position == "*", (
                    mutation + " has a * but not formatted like a wildcard"
                )
            ...
            if nucleotide_mutation:
                ...
                if wildcard:
                    ...
                    assert before in ["c", "t", "g", "a"], (
                        before + " is not a nucleotide!"
                    )

                    if position < 0:
                        mask = self.genome_feature_name == gene_name
                        lowest_promoter_number = numpy.min(
                            self.genome_nucleotide_number[mask]
                        )

                        if lowest_promoter_number > position:
                            mask = (self.genome_feature_name == gene_name) & (
                                self.genome_nucleotide_number == 1
                            )
                            index = self.genome_index[mask] + position
                            mask = self.genome_index == index
                            assert before == self.genome_sequence[mask], (
                                "specified nucleotide is "
                                + before
                                + " but is "
                                + self.genome_sequence[mask][0]
                            ...
                            alternative_variant = (
                                self.genome_feature_name[mask][0]
                                + "@"
                                + str(self.genome_nucleotide_number[mask][0])
                                + before
                                + ">"
                                + after
                            )
                            raise ValueError(
                                "genome index occupied by provided gene@mutation "
                                + mutation
    ...
    def deal_with_amino_acid_SNP(self, after, wildcard, before, cols, gene_name):
        ...
        assert after in [
            ...
            "X",
            "O",
        ], (after + " is not an amino acid or recognised character!")
        ...
        if not wildcard:
            ...
            assert before in [
                ...
                "W",
                "Y",
            ], (before + " is not an amino acid or recognised character!")
            ...
            assert numpy.count_nonzero(mask) == 1, (
                "position " + str(position) + " is not in the genome"
            )

            assert before == self.genes[gene_name].amino_acid_sequence[mask][0], (
                "specified amino acid is "
                + before
                + " but is "
                + self.genes[gene_name].amino_acid_sequence[mask][0]
                + " in the reference genome"
            )
    ...
    def deal_with_wildcards_for_INDEL(self, cols=None, mutation=None, gene_name=None):
        ...
        if cols is None or mutation is None or gene_name is None:
            raise ValueError("cols, mutation or gene_name is None")

        # deal with wildcards in the position
        if cols[0] in ["*", "-*"]:

            assert cols[1] in ["ins", "del", "indel", "fs"], (
                "INDEL must be on the format katG_*_fs i.e. the third element must be ins or del, not "
                + cols[2]
            )
            ...
            assert "*" not in cols[0], (
                "mutation " + mutation + " contains a wildcard (*) but is badly formed"
            )
            ...
            assert numpy.count_nonzero(mask) == 1, (
                "specified position " + cols[0] + " not in the gene: " + mutation
            )

            # be defensive here also!
            assert cols[1] in ["ins", "del", "indel", "fs"], (
                "INDEL must be on the format rpoB_1300_ins_1 i.e. the third element must be ins or del, not "
                + cols[2]
            )
            ...
            if len(cols) == 3:
                try:
                    ...
                    ), "an INDEL must be a non-zero number of bases"
                except:
                    assert bool(re.match("^[catg]+$", cols[2])), (
                        cols[2] + " INDEL contains bases other than a,t,c,g"
                    )
