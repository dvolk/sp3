...
class Gene(object):
    ...
    def __init__(
        ...
        assert codes_protein in [True, False], (
            gene_name + ": codes_protein must be True or False!"
        )
        self.codes_protein = codes_protein

        assert isinstance(sequence, numpy.ndarray), (
            gene_name + ": sequence of bases must be a Numpy array!"
        )

        assert isinstance(index, numpy.ndarray), (
            gene_name + ": genome indices must be a Numpy array of integers!"
        )
        assert numpy.issubdtype(index.dtype.type, numpy.integer), (
            gene_name + ": genome indices must be a Numpy array of integers!"
        )

        assert isinstance(numbering, numpy.ndarray), (
            gene_name + ": gene numbering must be a Numpy array of integers!"
        )
        ...
        assert numpy.count_nonzero(
            numpy.isin(sequence, ["a", "t", "c", "g", "x", "z", "o"])
        ) == len(sequence), (gene_name + ": sequence can only contain a,t,c,g,z,x")
    ...
    def __repr__(self):
        ...
        if self.codes_protein:
            ...
            output += "\n"
            output += "".join(
                str(i) + " " for i in self.amino_acid_numbering[:string_length]
            )
            output += "..."
            output += "".join(
                str(i) + " " for i in self.amino_acid_numbering[-string_length:]
    ...
    def list_mutations_wrt(self, other):
        ...
        assert (
            self.total_number_nucleotides == other.total_number_nucleotides
        ), "genes must have the same length!"
        assert self.gene_name == other.gene_name, "both genes must be identical!"
        assert (
            self.codes_protein == other.codes_protein
    ...
    def table_mutations_wrt(self, other):
        ...
        if len(MUTATIONS_dict["POSITION"]) > 0:
            ...
            MUTATIONS_table = MUTATIONS_table.astype(
                {
                    "GENE": "category",
                    "MUTATION": "str",
                    "REF": "str",
            ...
            MUTATIONS_table = MUTATIONS_table.replace(
                {
                    "POSITION": 0,
                    "NUCLEOTIDE_NUMBER": 0,
                    "AMINO_ACID_NUMBER": 0,
    ...
    def valid_variant(self, variant):
        ...
        assert before == self.sequence[mask][0], (
            "base in genome is "
            + self.sequence[mask][0]
