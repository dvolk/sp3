...
def process_catalogue_GARC1(rules, drugs, catalogue_genes_only):
    ...
    # infer these columns
    rules[
        ["GENE", "MUTATION", "POSITION", "MUTATION_AFFECTS", "MUTATION_TYPE"]
    ...
    if catalogue_genes_only:
        ...
        drug_list_index = (
            rules.loc[rules.PREDICTION == "R"][["DRUG", "GENE"]]
            .groupby(["DRUG", "GENE"])
            .count()
        )
...
def process_snp_variants(
    ...
    if before == after:
        ...
        row_prediction(
            row, predictions, 1, "syn SNP at any position in the CDS", verbose
        )
        ...
        if mutation_affects == "CDS":
            row = rules.loc[rules_mutation_type_vector & (rules.MUTATION == "*?")]
            row_prediction(
                row, predictions, 2, "nonsyn SNP at any position in the CDS", verbose
            )
        else:
            row = rules.loc[rules_mutation_type_vector & (rules.MUTATION == "-*?")]
            row_prediction(
                row, predictions, 2, "nonsyn SNP at any position in the PROM", verbose
            )
        ...
        if mutation[-1] in ["Z", "z", "O", "o", "X", "x"]:
            if mutation_affects == "CDS":
                if mutation[-1] == "Z":
                    ...
                    row_prediction(
                        row, predictions, 3, "null at any position in the CDS", verbose
                    )
            else:
                if mutation[-1] == "z":
                    row = rules.loc[
                    ...
                    row_prediction(
                        row, predictions, 3, "null at any position in the PROM", verbose
                    )
        ...
        if mutation[-1] in ["Z", "z"]:
            ...
            row_prediction(
                row, predictions, 7, "het SNP at specified position in the CDS", verbose
            )
        ...
        row_prediction(
            row, predictions, 8, "nonsyn SNP at specified position in the CDS", verbose
        )
...
def process_indel_variants(
    ...
    row_prediction(
        row, predictions, 2, "any insertion (or deletion) in the CDS or PROM"
    )
    ...
    if indel_length is not None and (indel_length % 3) != 0:
        ...
        row_prediction(
            row, predictions, 4, "any frameshifting insertion or deletion in the CDS"
        )
...
def parse_mutation(gene, mutation):
    ...
    if len(cols) == 1:
        ...
        assert indel_type in ["indel", "ins", "del", "fs"], (
        ...
        if len(cols) == 3:
            assert indel_type in ["ins", "del"], (
                "form of indel does not make sense when length or bases specified!: "
                + indel_type
            )
            try:
    ...
    assert mutation_type in ["INDEL", "SNP"], (
        "form of mutation_type not recognised: " + mutation_type
    )
...
def sanity_check_snp(before, after):
    ...
    assert after in [
        ...
        "Y",
        "Z",
    ], (after + " is not a recognised amino acid or base!")
    assert before in [
        "a",
        "c",
        "t",
        ...
        "W",
        "Y",
    ], (before + " is not a recognised amino acid or base!")
