...
catalogue = collections.namedtuple(
    "catalogue",
    [
        "genbank_reference",
        "name",
...
def load_catalogue(catalogue_file, prediction_subset_only):
    ...
    duplicated_rows = rules.loc[
        rules.duplicated(subset=["DRUG", "MUTATION"], keep="first")
    ]
    assert len(duplicated_rows) == 0, (
        "There are rows with duplicated [DRUG, MUTATION] entries! " + duplicated_rows
    )
