...
con.execute(
    "create table if not exists catgrid_stats (time, nodes, cores, mem, free_mem, running, queued, free_disk_root, free_disk_data, free_disk_work)"
)
...
def stat_collector():
    ...
    while True:
        ...
        con.execute(
            "insert into catgrid_stats values (?,?,?,?,?,?,?,?,?,?)",
            (
                now,
                n_nodes,
                cores,
...
def get_data():
    data = con.execute(
        "select * from catgrid_stats order by time desc limit 1440"
    ).fetchall()
    ...
    return json.dumps({"status": "success", "message": "", "data": ret})
