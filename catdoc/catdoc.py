...
def first_page(canvas, doc):
    if brand_org == "PHE":
        canvas.drawImage(
            "/home/ubuntu/sp3/catdoc/PublicHealthEngland.jpg",
            7 * mm,
            277 * mm,
            width=16 * mm,
            height=1 * cm,
        ...
        canvas.drawImage(
            "/home/ubuntu/sp3/catdoc/logo.png",
            7 * mm,
            277 * mm,
            width=2 * cm,
            height=1 * cm,
...
def go(report_json_path, pdf_out_path, brand=""):
    ...
    elements.append(
        Paragraph(f"Pipeline run ID: {data.get('run_uuid')}", top_para_style)
    )
    elements.append(Spacer(0, 3 * mm))
    ...
    if "mykrobe_speciation" in data:
        ...
        if type(tbl["phylo_group"]) == dict:
            for k, v in tbl["phylo_group"].items():
                data_.append(
                    ["Phylo group", k, v["percent_coverage"], v["median_depth"]]
                )
        ...
        if type(tbl["sub_complex"]) == dict:
            for k, v in tbl["sub_complex"].items():
                data_.append(
                    ["Sub-complex", k, v["percent_coverage"], v["median_depth"]]
                )
        t = Table(data_, hAlign="LEFT")
        t.setStyle(TableStyle([("GRID", (0, 0), (-1, -1), 0.25, colors.grey)]))
        elements.append(t)
    ...
    if "resistance" in data:
        ...
        data_ = [list(tbl["prediction_ex"].keys()), list(tbl["prediction_ex"].values())]
        t = Table(data_, hAlign="LEFT")
        t.setStyle(TableStyle([("GRID", (0, 0), (-1, -1), 0.25, colors.grey)]))
        elements.append(t)
        ...
        for item in tbl["mutations"]:
            if (
                item["mutation_name"][-1] not in ["z", "Z"]
                and item["mutation_name"][0] != item["mutation_name"][-1]
            ):
                for d in data["resistance"]["data"]["res_rev_index"][
                    item["gene_name"] + "_" + item["mutation_name"]
                ]:
    ...
    try:
        if "run_distmatrix" in data:
            ...
            elements.append(
                Paragraph(
                    f"Base counts: {s['counts']['N']} N, {s['counts']['T']} T, {s['counts']['C']} C, {s['counts']['G']} G, {s['counts']['A']} A"
                )
            )
            elements.append(
                Paragraph(f"FASTA distances within run", styles["Heading3"])
            )
