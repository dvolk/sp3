...
class ENA_Fetcher(threading.Thread):
    ...
    def get_metadata(self, accession, guid):
        ...
        '''
        ena_url = 'http://www.ebi.ac.uk/ena/data/warehouse/filereport?accession={0}&result=read_run'.format(accession)
        self.tlogger.debug(ena_url)
        ena_request = requests.get(ena_url)
        if ena_request.status_code != 200:
            self.tlogger.error("ena returned http code {0} for accession {1}".format(ena_request.status_code, accession))
    ...
    def get_files(self, tbl, fetch_range):
        ...
        for num, row in tbl.iterrows():
            ...
            if 'fastq_ftp' not in row or 'fastq_md5' not in row:
                self.tlogger.error('run accession {0} doesn\'t have fastq_ftp or fastq_md5'.format(run_accession))
            ...
            try:
                ...
                self.tlogger.error('run accession {0} doesn\'t have fastq_ftp or fastq_md5 (2)'.format(run_accession))
                bad_files.add(run_accession)
                continue

            if len(file_pair) != 2:
                self.tlogger.warning("run accession {0} doesn't have paired reads".format(run_accession))
                bad_files.add(run_accession)
                continue
            if len(md5_pair) != 2:
                self.tlogger.warning("run accession {0} doesn't have paired md5s".format(run_accession))
                bad_files.add(run_accession)
                continue
        ...
        self.tlogger.info("found {0}/{1} ok/bad files in metadata table".format(len(ok_files), len(bad_files)))
    ...
    def download_file(self, ftp_url, ftp_md5, tbl):
        ...
        if pathlib.Path(out_filepath).exists():
            ...
            if file_md5 == ftp_md5:
                self.tlogger.info("file {0} exists and matches metadata md5".format(out_filepath))
                return util.make_api_response(status='success')
            else:
                self.tlogger.warning("file {0} exists but does not match metadata md5".format(out_filepath))
        ...
        if file_md5 == ftp_md5:
            self.tlogger.info("md5 OK")
            return util.make_api_response(status='success', data={'output_file': out_filepath })
        else:
    ...
    def download_files(self, accession, guid, data):
        ...
        return util.make_api_response(status=status,
