    ...
    if queue.get_val(in_guid, "status") not in ["queued", "running"]:
        return json.dumps(
            util.make_api_response(
                "success", data={"state": "not in valid configuration"}
            )
        )
...
def fetch_log(in_guid):
    ...
    try:
        ...
        pass
    return json.dumps(
        util.make_api_response("success", data={"app": ret1, "ena": ret2})
    )
...
def fetch_new(fetch_kind):
    ...
    if fetch_kind == "ena2":
        ...
        if len(samples) > 0:
            ...
            return json.dumps(
                util.make_api_response(
                    "success", {"message": "No accessions detected."}, data={}
                )
            )
        return ret
...
def try_register_sp3data(args, in_accession):
    ...
    try:
        ...
        requests.post(
            "http://127.0.0.1:22000/load_sp3_data",
            headers={"content-type": "application/json"},
            data=json.dumps({"fetch_uuid": guid, "fetch_dir": directory}),
        )
