...
class SqliteQueue:
    def __init__(self, sqlite3_target):
        ...
        with self.lock, self.con:
            self.con.execute("create table if not exists q (guid primary key not null, status, name, data, progress int, total int, started int, finished, kind)")
            self.con.execute("create view if not exists blah as select guid, status, name, '', progress, total, started, finished, kind from q where status <> 'deleted' order by started asc")
    ...
    def push(self, kind, elem, guid, data, pred):
        ...
        '''
        with self.lock, self.con:
            existing = self.con.execute('select * from q where name = ? and kind = ? and status <> "failure" and status <> "deleted"', (elem,kind)).fetchall()
            if existing:
                valid = pred(existing)
                if valid:
                    return valid

            self.con.execute('insert into q values (?, ?, ?, ?, ?, ?, ?, ?, ?)', (guid, "queued", elem, data, 0, 0, time.time(), 0, kind))
            return None
    ...
    def get_val(self, guid, col):
        ...
        '''
        with self.lock, self.con:
            ret = self.con.execute('select {0} from q where guid = ?'.format(col), (guid,)).fetchall()
    ...
    def set_val(self, guid, col, new_val):
        ...
        '''
        with self.lock, self.con:
            ret = self.con.execute('update q set {0} = ? where guid = ?'.format(col), (new_val, guid))
    ...
    def apply_fun(self, guid, col, f):
        ...
        '''
        with self.lock, self.con:
            val = self.con.execute('select {0} from q where guid = ?'.format(col), (guid,)).fetchall()
            if val:
                val = val[0][0]
                new_val = f(val)
                self.con.execute('update q set {0} = ? where guid = ?'.format(col), (new_val, guid))
                return True
            else:
                return False
    ...
    def pop(self, kind):
        ...
        while True:
            with self.lock, self.con:
                elems = self.con.execute('select * from q where status = "queued" and kind = ?', (kind,)).fetchall()
                if elems:
                    guid, name, data = elems[0][0], elems[0][2], elems[0][3]
                    self.con.execute('update q set status = "running" where guid = ?', (guid,))
        ...
        with self.con:
            ...
            for guid, status, name, data, progress, total, started, finished, kind in elems:
                ret[guid] = { 'status': status, 'name': name, 'data': data,
                              ...
                              'started': started, 'finished': finished, 'kind': kind }
        return ret
