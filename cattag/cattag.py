...
con.execute(
    "create table if not exists run_tags (pipeline_name, run_uuid, tag_type, tag_name, unique(pipeline_name, run_uuid, tag_type, tag_name))"
)
con.execute(
    "create table if not exists sample_tags (pipeline_name, run_uuid, sample_name, tag_type, tag_name, unique(pipeline_name, run_uuid, sample_name, tag_type, tag_name))"
)
...
def get_run_tags(pipeline_name, run_uuid):
    data = con.execute(
        "select pipeline_name, run_uuid, tag_type, tag_name from run_tags where pipeline_name = ? and run_uuid = ?",
        (pipeline_name, run_uuid,),
    ).fetchall()
    return make_api_response(status="success", details=None, data=data)


@app.route("/get_sample_tags_for_run/<run_uuid>")
def get_sample_tags_for_run(run_uuid):
    data = con.execute(
        "select sample_name, tag_type, tag_name from sample_tags where run_uuid = ?",
        (run_uuid,),
    ).fetchall()
    return make_api_response(status="success", details=None, data=data)
...
def get_sample_tags(run_uuid, sample_name):
    data = con.execute(
        "select run_uuid, tag_type, tag_name from sample_tags where run_uuid = ? and sample_name = ?",
