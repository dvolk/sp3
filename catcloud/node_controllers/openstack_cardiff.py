...
def os_create_volume(name, size, image):
    out = run(f'openstack volume create --size {size} --type ceph --image {image} {name} -f json')
...
def os_create_server(name, flavor, network_uuid, key_name, vol_uuid):
    out = run(f'openstack server create --volume {vol_uuid} --flavor {flavor} --key-name {key_name} --nic net-id={network_uuid} {name} -f json')
    return json.loads(out)
...
class OpenstackCardiffNodeController:
    def __init__(self, flavor, network_uuid, key_name, vol_size, vol_image, setup_script):
        self.flavor = flavor
    ...
    def create(self):
        server_ip = os_make_node(self.vol_size, self.vol_image, self.key_name, self.flavor, self.network_uuid, self.setup_script)
        return server_ip
