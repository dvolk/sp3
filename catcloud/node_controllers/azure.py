...
def az_create_vm(name, resource_group, image, instance_type):
    ret = run(f'az vm create -g {resource_group} --name {name} --size {instance_type} --image {image} --output json --public-ip-address ""')
...
class AzureNodeController:
    ...
    def create(self):
        return az_create_node(self.instance_type, self.resource_group, self.vm_image, self.setup_script)
