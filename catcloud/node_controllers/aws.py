...
def aws_create_vm(image_id, instance_type, key_name, security_group_id, subnet_id):
    ret = run(f'aws ec2 run-instances --image-id {image_id} --count 1 --instance-type {instance_type} --key-name {key_name} --security-group-id {security_group_id} --subnet-id {subnet_id}')
    return json.loads(ret)

def aws_create_node(image_id, instance_type, key_name, security_group_id, subnet_id, setup_script):
    node_js = aws_create_vm(image_id, instance_type, key_name, security_group_id, subnet_id)
...
class AWSNodeController:
    def __init__(self, image_id, instance_type, key_name, security_group_id, subnet_id, setup_script):
    ...
    def create(self):
        return aws_create_node(self.image_id, self.instance_type, self.key_name, self.security_group_id, self.subnet_id, self.setup_script)
