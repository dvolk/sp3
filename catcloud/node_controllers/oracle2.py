...
def oracle_get_server_by_ip(server_ip, subnet_id):
    r = run(f'oci network private-ip list --subnet-id {subnet_id} --auth instance_principal')
...
def oracle_get_instance_id_from_name(display_name, compartment_id):
    vms = run(f'oci compute instance list -c { compartment_id } --auth instance_principal')
    for vm in json.loads(vms)['data']:
        if vm['display-name'] == display_name:
            return vm['id']

def oracle_create_vm(name, compartment_id, availability_domain, image_id, shape, subnet_id, boot_volume_size_in_gbs):
    cmd = f"""
...
def oracle_create_node(compartment_id, availability_domain, image_id, shape, subnet_id, boot_volume_size_in_gbs, setup_script):
    node_uuid = str(uuid.uuid4())
    node_js = oracle_create_vm(node_uuid, compartment_id, availability_domain, image_id, shape, subnet_id, boot_volume_size_in_gbs)
...
def oracle_destroy_node(server_ip, subnet_id, compartment_id):
    ...
    run(f'oci compute instance terminate --force --instance-id { instance_id } --preserve-boot-volume false --auth instance_principal')

class OracleNodeController:
    def __init__(self, compartment_id, availability_domain, image_id, shape, subnet_id, boot_volume_size_in_gbs, setup_script):
    ...
    def create(self):
        return oracle_create_node(self.compartment_id, self.availability_domain, self.image_id, self.shape, self.subnet_id, self.boot_volume_size_in_gbs, self.setup_script)
