...
def is_token_valid(token):
    ...
    if (
        token_last_active_age > token_last_active_age_max
        or token_added_age > token_added_age_max
    ):
        return False
    if token_last_active_age < 0 or token_added_age < 0:
        return False

    tokens_db.update_one(
        {"token_id": token}, {"$set": {"last_active_time": int(time.time())}}
    )
    return True
...
def edit_user():
    ...
    accounts_db.update_one({"username": username}, {"$set": user_data})
    return "OK"
...
def edit_organisation():
    ...
    organisations_db.update_one({"name": name}, {"$set": org_data})
    return "OK"
...
def change_password(token_id):
    ...
    accounts_db.update_one(
        {"username": username}, {"$set": {"password_hash": new_pw_hash}}
    )
