...
def group_reports_by_sample(reports_rows):
    ...
    for report_row in reports_rows:
        sample_unique_id = report_row['pipeline_run_uuid'] + '/' + report_row['sample_name']
...
def extract_kraken2_data(report_row):
    tbl = pandas.read_csv(report_row['report_filename'],
                          sep='\t',
                          header=None,
                          names=['RootFragments%',
                                 'RootFragments',
    ...
    data['human'] = tbl.query('Name == "Homo sapiens"').head(n=1).to_dict(orient='records')
    data['mycobacteriaceae'] = tbl.query('Name == "Mycobacteriaceae"').head(n=1).to_dict(orient='records')
    return data
...
def process_samples(samples):
    dispatch = { 'pick_reference': extract_pick_reference_data,
...
def insert_samples_sqlite(samples):
    ...
    for unique_id, report_data in reports_data.items():
        ...
        con2.execute('insert into test (pipeline_run_uuid, sample_name) values (?, ?)', (pipeline_run_uuid, sample_name))
        if 'samtools_qc' in report_data:
            samtools_qc_cols = ['samtools_qc_raw_total_sequences',
            ...
            samtools_qc_keys = ['raw total sequences:',
                                'reads mapped:',
                                'reads paired:',
            ...
            con2.execute(f'update test set {samtools_cols_stmt} where pipeline_run_uuid = ? and sample_name = ?',
                         (*[report_data['samtools_qc'][x] for x in samtools_qc_keys], pipeline_run_uuid, sample_name))

        if 'resistance' in report_data:
            resistance_keys = report_data['resistance'].keys()
            resistance_cols = "".join(['resistance_' + x + ' = ?,' for x in resistance_keys if 'SNPIT' not in x])[:-1]
            if resistance_keys:
                # print(report_data)
                params = (*[report_data['resistance'][x] for x in resistance_keys if 'SNPIT' not in x], pipeline_run_uuid, sample_name)
                # print(params)
                # print('\n')
                # print(resistance_cols)
                con2.execute(f'update test set {resistance_cols} where pipeline_run_uuid = ? and sample_name = ?', params)

        if 'kraken2_speciation' in report_data:
            for k,v in report_data['kraken2_speciation'].items():
                if v:
                    con2.execute(f'update test set kraken2_{k} = ?, kraken2_{k}_direct_fragments = ?, kraken2_{k}_root_fragments = ? where pipeline_run_uuid = ? and sample_name = ?',
                                 (v[0]['Name'], v[0]['RootFragments'], v[0]['DirectFragments'], pipeline_run_uuid, sample_name))

        if 'mykrobe_speciation' in report_data:
            for k,v in report_data['mykrobe_speciation'].items():
                for k2,v2 in v.items():
                    con2.execute(f'update test set mykrobe_{k} = ?, mykrobe_{k}_percent_coverage = ?, mykrobe_{k}_median_depth = ? where pipeline_run_uuid = ? and sample_name = ?',
                             (k2, v2['percent_coverage'], v2['median_depth'], pipeline_run_uuid, sample_name))

        if 'pick_reference' in report_data:
            con2.execute('update test set pick_reference_tax_id = ? where pipeline_run_uuid = ? and sample_name = ?',
                         (report_data['pick_reference']['pick_taxid'], pipeline_run_uuid, sample_name))
...
def insert_samples_pymysql(samples):
    con3 = pymysql.connect(host='10.151.229.64',
                           user='denis',
    ...
    for unique_id, report_data in reports_data.items():
        pipeline_run_uuid, sample_name = unique_id.split('/')
        con2.execute('insert into lsb_reports (pipeline_run_uuid, sample_name) values (%s, %s)', (pipeline_run_uuid, sample_name))
        if 'samtools_qc' in report_data:
            samtools_qc_cols = ['samtools_qc_raw_total_sequences',
                                'samtools_qc_reads_mapped',
                                'samtools_qc_reads_paired',
            ...
            con2.execute(f'update lsb_reports set {samtools_cols_stmt} where pipeline_run_uuid = %s and sample_name = %s',
                         (*[report_data['samtools_qc'][x] for x in samtools_qc_keys], pipeline_run_uuid, sample_name))

        if 'resistance' in report_data:
            resistance_keys = report_data['resistance'].keys()
            resistance_cols = "".join(['resistance_' + x + ' = %s,' for x in resistance_keys if 'SNPIT' not in x])[:-1]
            if resistance_keys:
                params = (*[report_data['resistance'][x] for x in resistance_keys if 'SNPIT' not in x], pipeline_run_uuid, sample_name)
                con2.execute(f'update lsb_reports set {resistance_cols} where pipeline_run_uuid = %s and sample_name = %s', params)

        if 'kraken2_speciation' in report_data:
            for k,v in report_data['kraken2_speciation'].items():
                if v:
                    con2.execute(f'update lsb_reports set kraken2_{k} = %s, kraken2_{k}_direct_fragments = %s, kraken2_{k}_root_fragments = %s where pipeline_run_uuid = %s and sample_name = %s',
                                 (v[0]['Name'], v[0]['RootFragments'], v[0]['DirectFragments'], pipeline_run_uuid, sample_name))

        if 'mykrobe_speciation' in report_data:
            for k,v in report_data['mykrobe_speciation'].items():
                for k2,v2 in v.items():
                    con2.execute(f'update lsb_reports set mykrobe_{k} = %s, mykrobe_{k}_percent_coverage = %s, mykrobe_{k}_median_depth = %s where pipeline_run_uuid = %s and sample_name = %s',
                             (k2, v2['percent_coverage'], v2['median_depth'], pipeline_run_uuid, sample_name))

        if 'pick_reference' in report_data:
            con2.execute('update lsb_reports set pick_reference_tax_id = %s where pipeline_run_uuid = %s and sample_name = %s',
                         (report_data['pick_reference']['pick_taxid'], pipeline_run_uuid, sample_name))
